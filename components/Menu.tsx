...
export default function Menu({ items }: { items: MenuItem[] }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <div className="rounded-full hoverable:hover:bg-black/10 active:bg-black/10 cursor-pointer w-6 h-6 flex justify-center items-center" onClick={onClick}>
      ...
      {isOpen && <div className="absolute top-4 right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button">
        {items.map(item => (
          <div className="py-1" role="none" key={`menu-item-${item.text}`}>
            <div className="block px-4 py-2 text-sm cursor-pointer hoverable:hover:bg-slate-300 active:bg-slate-300" role="menuitem" id="menu-item-0"
              onClick={(e) => {
                item.onClick();
                setIsOpen(false);
              }}
